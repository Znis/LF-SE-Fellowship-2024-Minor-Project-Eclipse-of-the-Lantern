...
export class BulletProjectile{ 
    ...
    show(ctx: CanvasRenderingContext2D = stateVariables.ctx){
        ...
        drawEllipse(ctx, this.startPoint.x, this.startPoint.y, this.r * 1.5, this.r * 1.5);

        ctx.fillStyle = 'rgba(255, 170, 51, 0.016)';
        for (let i = 0; i < this.r * 4; i++) {
            drawEllipse(ctx, this.initialX - 10, this.initialY, i * 3, i * 3);
        }

        ctx.save();

        ctx.translate(this.startPoint.x, this.startPoint.y);
        ctx.rotate(Math.atan2(this.startPoint.y - this.endPoint.y, this.startPoint.x - this.endPoint.x));

        for (let i = 0; i < this.r * 4; i++) {
            drawEllipse(ctx, 0, 0, i * 5, i);
        }
