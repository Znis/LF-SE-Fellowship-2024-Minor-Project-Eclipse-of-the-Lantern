...
export class Axe{
    ...
    show(ctx: CanvasRenderingContext2D = stateVariables.ctx, isAttacking: boolean){
        ...
        if(stateVariables.player.direction == "r"){
            ...
            let position = isAttacking ? Math.floor(this.spritePos / staggerFrame) % 8 : 0;
            ctx.drawImage(
              axeRight.sprite,
            ...
            let position = isAttacking ? Math.floor(this.spritePos / staggerFrame) % 3 : 0;
            ctx.drawImage(
              axeBack.sprite,
    ...
    calculateShockPoint(){
        const angle = Math.atan2((stateVariables.mouseCoords.y - stateVariables.player.startPoint.y) , (stateVariables.mouseCoords.x - stateVariables.player.startPoint.x));
        const dy = Math.sin(angle) * this.r;
        const dx = Math.cos(angle) * this.r;
        this.shockPoint = new Point(this.startPoint.x + dx + 20, this.startPoint.y + dy + 50);
